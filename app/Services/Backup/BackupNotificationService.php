...
class BackupNotificationService
{
    ...
    public function createSystemNotification(Backup $backup, string $type): void
    {
        ...
        $this->notificationService->send(
            ...
            $title,
            $message,
            null, // icon - سيتم استخدام الافتراضي
            route('backups.show', $backup->id), // action_url
            'App\Models\Backup', // related_type
            $backup->id, // related_id
            [
                'backup_id' => $backup->id,
                'url' => route('backups.show', $backup->id),
            ] // metadata
        );
    }
}
