...
/**
 * Makes model-written HTML safe to store and render.
 *
 * Three failures reached the database before this existed:
 *   - a section truncated mid-tag left <section> unclosed, so every later section
 *     nested inside it and the page rendered as one run-on block;
 *   - code blocks arrived on a single line (the JSON envelope the prompts used to
 *     require made models avoid newlines), which breaks any sample using `//`;
 *   - a truncated JSON envelope left a literal `" }` paragraph at the end.
 *
 ...
 */
class DocumentationHtmlRepairer
{
    ...
    /** Repair one generated section and guarantee its outer shape. */
    ...
    /** Repair a whole assembled page, leaving its section structure as-is. */
    ...
    private function load(string $html): ?DOMDocument
    {
        ...
        $ok = $dom->loadHTML(
            ...
            .'<div id="docs-repair-root">'.$html.'</div>',
    ...
    {
        $found = (new DOMXPath($dom))->query('//div[@id="docs-repair-root"]');
    ...
    /**
     * Guarantee exactly one top-level <section class="content-section"> carrying
     * one <h2 class="section-title">.
     */
    private function normalizeSectionShape(DOMDocument $dom, string $heading): void
    {
        ...
        // A section wrapping nothing but another section: the model echoed the
        // wrapper the prompt showed it.
        ...
        $sections = $xpath->query('./section', $root);
        $sectionCount = $sections->length;
        $section = null;

        if ($sectionCount === 1 && $sections->item(0) instanceof DOMElement) {
            $section = $sections->item(0);
            // Content that leaked outside the section belongs inside it.
            $this->absorbSiblings($root, $section);
        } else {
            // Zero sections (a bare fragment) and several (the model ignored
            // "one section only") both collapse into a single wrapper.
            ...
            if ($sectionCount > 1) {
                foreach (iterator_to_array($xpath->query('.//section', $section)) as $inner) {
                    if ($inner instanceof DOMElement) {
                        $this->unwrap($inner);
                    }
                }
        ...
        $this->ensureHeading($dom, $section, $heading);
    }

    /** Move nodes that ended up as siblings of the section inside it. */
    private function absorbSiblings(DOMElement $root, DOMElement $section): void
    {
    ...
    private function ensureHeading(DOMDocument $dom, DOMElement $section, string $heading): void
    {
        $headings = (new DOMXPath($dom))->query('.//h2', $section);
        ...
        foreach (iterator_to_array($headings) as $index => $h2) {
            ...
            // demote them to the subsection style instead.
            $this->renameTo($dom, $h2, 'h3', 'subsection-title');
        }
    }

    private function renameTo(DOMDocument $dom, DOMElement $element, string $tag, string $class): void
    {
