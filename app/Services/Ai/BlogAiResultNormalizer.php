...
/**
 * Unwraps AI blog-post payloads that leak JSON into HTML fields,
 * and validates wizard-shaped results before they reach the admin form.
 *
 * Identical logic to DocumentationAiResultNormalizer — nothing here is specific
 * to documentation pages (generic JSON unwrapping / HTML-shape detection), kept
 * as its own class per-feature to avoid a cross-feature dependency.
 */
class BlogAiResultNormalizer
{
    /**
    ...
    public function assertWizardShape(array $result, ?string $topic = null): array
    {
        ...
        if ($content === '' || ! $this->isPlausibleHtml($content)) {
            throw new RuntimeException(
                'استجابة التوليد لا تحتوي HTML صالحاً للمقال. أعد المحاولة — لا تُحفظ حقول JSON خام.'
            );
        }
        ...
        if ($title === '' || $this->looksLikeInstructionPrompt($title, $topic) || $this->looksLikeJsonBlob($title)) {
            throw new RuntimeException(
                'عنوان المقال غير صالح (يشبه الطلب أو JSON). أعد التوليد.'
            );
        }
