...
class GroupRegistrationReceiptService
{
    ...
    public function store(UploadedFile $receipt, int $groupId): string
    {
        $directory = 'group-registrations/payment-receipts/' . date('Y') . '/' . $groupId;
        ...
        if (! $storedPath) {
            throw new RuntimeException('تعذر رفع وصل الانتساب. يرجى المحاولة مرة أخرى.');
        }
