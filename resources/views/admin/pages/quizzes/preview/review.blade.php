...
@section('content')
    <div class="main-content app-content student-quiz-review-page">
        <div class="container-fluid">
            ...
            <div class="alert alert-info border-info d-flex align-items-center gap-2 mb-4" role="alert">
                <i class="fe fe-eye fs-4"></i>
                <div>
                    <strong>نتيجة تجربة المعاينة</strong> — هذه المحاولة لا تظهر في إحصائيات الطلاب.
                </div>
            </div>
            ...
            <div class="d-md-flex d-block align-items-center justify-content-between my-4">
                <div class="min-w-0">
                    <h4 class="student-quizzes-welcome__title mb-1 text-truncate">
                        {{ $attempt->quiz->title }} — معاينة #{{ $attempt->attempt_number }}
                    </h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0">
                            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">لوحة التحكم</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('quizzes.index') }}">الاختبارات</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('quizzes.show', $quiz->id) }}">{{ $attempt->quiz->title }}</a></li>
                            <li class="breadcrumb-item active">مراجعة المعاينة</li>
                        </ol>
                    </nav>
                </div>
                <div class="mt-3 mt-md-0 d-flex flex-wrap gap-2">
                    <form method="POST" action="{{ route('quizzes.preview.start', $quiz->id) }}" class="d-inline">
                        ...
                        <button type="submit" class="btn btn-warning rounded-pill">
                            <i class="fe fe-refresh-cw me-1"></i>تجربة جديدة
                        </button>
                    </form>
                    <a href="{{ route('quizzes.show', $quiz->id) }}" class="btn btn-outline-primary rounded-pill">
                        <i class="fe fe-arrow-right me-1"></i>عودة للاختبار
                    </a>
                </div>
            </div>
            ...
            <div class="card custom-card student-quizzes-panel mt-2">
                <div class="card-body">
                    <div class="d-flex justify-content-between flex-wrap gap-2">
                        <a href="{{ route('quizzes.show', $quiz->id) }}" class="btn btn-outline-secondary rounded-pill">
                            <i class="fe fe-arrow-right me-1"></i>العودة لتفاصيل الاختبار
                        </a>
                        <form method="POST" action="{{ route('quizzes.preview.start', $quiz->id) }}" class="d-inline">
                            @csrf
                            <button type="submit" class="btn btn-warning rounded-pill">
