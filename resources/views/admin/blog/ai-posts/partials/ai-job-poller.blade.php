{{-- Shared async blog-post AI job polling — forked from admin.docs.pages.partials.ai-job-poller
     (identical module, only the three route names differ) --}}
<script>
window.BlogAiJobPoller = (function () {
    const statusUrlTemplate = @json(route('admin.blog.ai-posts.jobs.show', ['uuid' => '__UUID__']));
    const resumeUrlTemplate = @json(route('admin.blog.ai-posts.jobs.resume', ['uuid' => '__UUID__']));
    const partialUrlTemplate = @json(route('admin.blog.ai-posts.jobs.partial', ['uuid' => '__UUID__']));
